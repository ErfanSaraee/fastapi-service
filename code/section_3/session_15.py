...
@app.get("/names")
def get_names(q: Annotated[str, Query(deprecated=True, alias="name", description="The name to search for", example="Amin", max_length=50)] = None):
    if q:
        return [name for name in names_list if q.lower() in name["name"].lower()]
    return names_list
...
def add_name(name: str = Body(..., min_length=1, max_length=50, embed=True)):
...
def update_name(name_id: int = Path(..., title="The ID of the name to get", ge=1), name: str = Form(...)):
