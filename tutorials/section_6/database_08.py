...
SQLALCHEMY_DATABASE_URL = "sqlite:///./sqlite_08.db"
...
enrollments = Table(
    ...
    UniqueConstraint("user_id", "course_id",
                     name="unique_user_course_enrolled"),
)
...
class User(Base):
    ...
    courses = relationship(
        "Course", secondary=enrollments, backref="attendees")
...
course = session.query(Course).filter_by(title="course 1").one()


# session.add(Course(title="course 1", description="description 1"))
# session.commit()
